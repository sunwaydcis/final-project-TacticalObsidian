...
class Board:
  ...
  private val _piecePositions = Map[B_ChessPieces, Coordinate]()
  ...
  def positionOf(piece: B_ChessPieces) = _piecePositions.get(piece)

  def pieceOn(row: Int, col: Int) = _board(row)(col)

  def movePiece(piece: B_ChessPieces, newRow: Int, newCol: Int) =
    println("I'm called movePiece()")
    removePiece(piece)
  ...
  private def removePiece(piece: B_ChessPieces) =
    val position: Option[(Int, Int)] = piecePositions.get(piece)
      _board(position.get(0))(position.get(1)) = None
      _piecePositions - piece
