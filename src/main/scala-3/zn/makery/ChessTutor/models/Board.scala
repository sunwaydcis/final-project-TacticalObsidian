...
class Board:
  var board: Array[Array[Option[BaseChessPiece]]] = Array.fill(8, 8)(None)
  var piecePositions = Map[BaseChessPiece, Coordinate]()
  private var possibleMoves = Map[BaseChessPiece, List[Int]]()
  ...
  def position(piece: BaseChessPiece) = piecePositions.get(piece).get

  def piece(row: Int, col: Int) = board(row)(col)

  def movePiece(piece: BaseChessPiece, newRow: Int, newCol: Int) =
  ...
  private def placePiece(piece: BaseChessPiece, row: Int, col: Int) =
    board(row)(col) = Some(piece)
    piecePositions += (piece -> (row, col))

  private def removePiece(piece: BaseChessPiece, row: Int, col: Int) =
    board(row)(col) = None
