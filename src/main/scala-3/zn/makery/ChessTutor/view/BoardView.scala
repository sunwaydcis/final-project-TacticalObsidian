...
//        println(s"Cell contains piece: ${piece._symbol} ${piece.getClass}")
  ...
  private def loadNewMove(piece: B_ChessPieces, newRow: Int, newCol: Int): Unit =
    ...
    val takes =
      board.pieceOn(newRow, newCol) match
        case Some(piece) =>
          moves = 0
          Some(piece)
        case None =>
  ...
  private def kingDead(piece: B_ChessPieces, newRow: Int, newCol: Int): Unit =
    ...
    opposingKingPosition match
      case Some((kingRow, kingCol)) =>
        ...
        if end == (newRow, newCol) then
          val winnerName = piece.color match
            case Alliance.White => game.whiteName
            case Alliance.Black => game.blackName
          ChessTutorApp.winDialog(s"$winnerName Won!")
      case None =>
